...
"""
fine_match.py (v4.1)
- 在粗配中心附近开一个小 ROI 做特征匹配
- 细配只用来重新排序候选，不再大幅度“跳位”
- 最终中心 = coarse_center 与 refined_center 的加权融合
- 加强中心位移惩罚，避免跑到 ROI 边缘的高纹理点上
"""
...
def load_yaml(p, default_={}):
    ...
    with open(p, "r", encoding="utf-8") as f:
        return yaml.safe_load(f) if yaml is not None else default_
...
def match_and_score(intra_gray, cand, templates_dir, meta, params, debug_dir=None):
    ...
    if kq is None or dq is None or len(kq) < 8:
        return None
    ...
    for tdir in tdirs:
        ...
        # 如果偏移太大（> max_shift_r），判为不可信：典型情况就是“跳到 ROI 边缘高纹理点”
        if center_cfg.get("enable", True):
...
def main():
    ...
    for i, c in enumerate(tqdm(cands, desc="candidates")):
        ...
        if ddir:
            ensure_dir(ddir)
        res = match_and_score(img, c, args.templates, meta, params, debug_dir=ddir)
        if res is None:
            continue
    ...
    if best_overall is None:
        raise RuntimeError(
            "No valid match. Try adjusting center.max_shift_r / center.alpha or score_weights."
        )
    ...
    print("fine_match: best ->", best_overall)
